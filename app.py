...
# ───────────────────────────────────────────────────────────────────────────────
# 2) Load the catalog.json and labelled examples (Excel)
# ───────────────────────────────────────────────────────────────────────────────
@st.cache_data
...
def load_labelled(path="Labelled Expressions.xlsx"):
    ...
    # Ensure columns are exactly "NLP Description" and "Expression"
...
# ───────────────────────────────────────────────────────────────────────────────
# 3) Few-shot helpers (pick examples by keyword overlap)
# ───────────────────────────────────────────────────────────────────────────────
def pick_few_shot_examples(query: str, df: pd.DataFrame, k: int = 6) -> pd.DataFrame:
    """
    Pick up to k rows from df whose 'NLP Description' shares the most words with query.
    If fewer than k match, randomly fill the remainder.
    """
...
def build_few_shot_prompt(query: str, df_examples: pd.DataFrame, k: int = 6) -> str:
    """
    Format k few-shot examples plus the user’s query into the multi-example prompt.
    """
...
# ───────────────────────────────────────────────────────────────────────────────
# 4) The core function: send few-shot prompt and return plain-text CentricExpr
# ───────────────────────────────────────────────────────────────────────────────
def nl2centricexpr_plain_few_shot(
    ...
    system_message = (
        ...
        "CentricExpr function call (no backticks, no extra text). If no valid function applies, "
        "output 'UNKNOWN()'."
    )
    ...
    # Strip any enclosing triple-backticks if they appear
...
st.write("Type a natural-language query below and click the button to get a CentricExpr expression.")
...
if submit:
    ...
    if not user_q:
        ...
        with st.spinner("Generating CentricExpr…"):
            try:
                result = nl2centricexpr_plain_few_shot(user_q, model=model_choice, few_shot_k=few_k)
